...
<!-- saliently Marisa Casillas, Elika Bergelson, Florencia Lopez Boo, Sarah Walker, Pauline Grosjean, Heidi Colleran, Jonathan Stieglitz, Gandhi Yetish. -->
...
A possibility is that you create a system of straps that kind of looks like a bra. One researcher that uses them is Marisa Casillas, so you can check her papers in the references, where she has a picture of this: it has a pocket on the front where you can put the device. Since it uses less material, it's typically easier to make and it is cheaper. In addition, you can make the straps elastic, which will allow you to fit children of different ages. It can be used in very warm and very cold climates.
...
2. Place the recorder in the outermost layer of clothing possible to aovid muffled sounds.

<!--chapter:end:02c-clothing.Rmd-->

# LENA software {#lena}

In this video, we are only going to discuss the LENA software. We discuss alternatives to the LENA software in another dedicated video \@ref(nonlena).
...
So if you are not working with English-learning infants 0-3 years, then the first thing you should oso if you are working with a population that speaks one of these languages, the LENA software might provide you with inaccurate or inappropriate measurements 
...
ChildProject is a package to manage daylong recordings. It is written in python, but it can also be used from the command line so that no knowledge of python required. However, notice that this does presupposed that you know your way around a terminal -- if you have no idea what that means, we strongly encourage you to take the online and free tutorial by Software Carpentry in the resources section. As we said in another video,  people who know another computer language (like python or R) learn bash basics in less than an hour. And if you don't know other languages, it may take you 3-4 hours, so it's still not very time consuming.
...
summarize: https://repositories.lib.utexas.edu/bitstream/handle/2152/83853/Preprint_MinorRevisions_Manuscript_ParentPerceptions.pdf?sequence=2
...
We have found it useful to provide parents with both oral and written instructions on everything they need to know: how the device works, how to turn it on, how they need to attach it to the clothing, how to recharge it (if you are recording multiple days), and that they can just go about their day. The written version could be provided printed out as well as on a website. This is useful because you can also have FAQs at the end, which will be informed by the things that the representatives of the community told you during your piloting sessions (see Video \@ref{pilot}). This may be how the data are going to be analysed, how they are going to be used for, etc. So anything parents told you or asked you about in the piloting stage, you can have it in a little booklet. 

The booklet is useful to some parents who like to consult it and have everything written down. However, for some things, it may be better to have short videos that are accessible through a QR code that the parents can scan in the booklet and so in this case they can read the explanation or watch the video. Particularly, regarding how to put the device in clothing, the booklet has pictures but nothing really replaces a tutorial video, so if you can do that, it will probably be appreciated. 

Ideally, you should also have a phone number they can call or send a message to, for instance via WhatsApp, if that is common in the community you are working with. This is very reassuring in case they have questions not on the booklet, or they want to refer someone else to you, for instance because they were incidentally recorded.
...
tab$repository<-c("Open Science Framework (osf.io)", "Databrary (databrary.org)", "Language Archive")
tab$ease<-c("Highest", "Intermediate", "Lowest")
tab$what<-c("Needs to create an account, use any format you want", "Needs to get institution approved & create an account, use their data format", "Needs to create an account, choice between their format or free-form")
tab$reusers<-c("Needs to create an account & get access from a curator", "Needs to get institution approved, create an account & get access from a curator", "Needs to create an account & get access from a curator (by signing a license)")
tab=data.frame(tab)
knitr::kable(tab, "pipe", col.name=c("Repository", "Ease of use", "What you, as curator, need to do", "What re-users need to do"), align=c("l", "l", "l", "l"))
...
For instance, imagine that you were trying to decide whether the child little or a lot based on sections that have been selected because the child talks -- that would make it difficult to know because you've purposefully selected regions where they are talking, giving you a sort of ceiling effect. 
...
You may still be thinking of LENA. Another issue we have come across with using LENA in the field is that it requires quite a bit of energy to be recharged, even if it is recharged from a USB port. Moreover, transferring the data is not that fast, and you need to do something special in your computer to extract the recording, which will not be immediately available to you. That is, even with a PRO License (which allows you to use the software in a portable computer), you don't extract the recording directly, but instead, first need to extract the data, then analyze it (which takes several hours of processing), and only then can you export the audio data. In our experience, we do not take all of these steps because the field sites we work in are not on the grid. Instead, we only take the step of transferring data from the device to the computer, and leave the processing and export for when we are back on the grid.

## Clothing

Regarding clothing, using paper or money clips to attach the device to the child's clothing is probably your top option. That said, if the regular clothing is loose, then we don't recommend using a money clip. As explained in the Clothing video, this will result in relatively poor data quality.
